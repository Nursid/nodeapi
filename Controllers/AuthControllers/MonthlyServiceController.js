...
const AddMonthlyService = async (req, res) => {
	...
	try {
		...
		const servicep_ids = await Promise.all(multiServiceProvider.map(async (providerName) => {
		...
		if (["Weekly", "Daily", "Alternative"].includes(data.serviceServeType)) {
			...
			switch (data.serviceServeType) {
				case "Weekly": incrementDays = 7;
					break;
				case "Alternative": incrementDays = 2;
					break;
				case "Daily": incrementDays = 1;
					break;
				default: incrementDays = 0; // Shouldn't happen
					break;
			}
			...
			for (const entry of availabilityEntries) { // Check if a record with the same emp_id and date already exists
				...
				if (existingEntry) { // If it exists, update the record
			...
			for (const supervisorEntry of availabilitySupervisor) { // Check if a record with the same emp_id and date already exists
				...
				if (existingSupervisor) { // If it exists, update the record
